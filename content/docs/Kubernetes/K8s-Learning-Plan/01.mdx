...
### 4.6 HA 负载均衡配置 (Keepalived & HAProxy)
...
## 5. 深度解析: kubeadm init 初始化流程与镜像优化
...
### 5.2 大陆环境镜像优化 (Offline/Proxy-free)
默认情况下, `kubeadm` 会访问 `registry.k8s.io`, 这在大陆环境通常无法连接.

*   **配置优化**: 如 4.6 节所示, 在 `ClusterConfiguration` 中设置 `imageRepository: registry.aliyuncs.com/google_containers`.
*   **查看所需镜像列表**:
    ```bash
...
### 8.2 核心原因分析: Sandbox Image (Pause 镜像)
...
### 8.3 核心原因分析: VIP 连通性 (Context Deadline Exceeded)
