...
kubectl get service -n monitoring-dev
kubectl get svc -n monitoring-dev -o wide
kubectl get svc -n monitoring-dev -l cnpg.io/cluster=pg-grafana
kubectl create secret generic pg-grafana-creds \\n  --from-literal=username=grafana_user \\n  --from-literal=password=Grafana123 \\n  -n monitoring-dev

kubectl get secret pg-grafana-creds -n monitoring-dev -o yaml
kubectl get secret pg-grafana-creds -n monitoring-dev --output jsonpath='{.data}'
kubectl get secret pg-grafana-creds -n monitoring-dev --output jsonpath='{.data}' | base64 --decode
```
