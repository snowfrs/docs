...
```mermaid
graph TD
    Client_Mail["邮件客户端 (Outlook/Apple)"] --> |SMTP 587/465| Postfix
    Client_Web[Web浏览器] -->|HTTPS 443| Nginx
    Client_Mobile["手机 (iOS/Android)"] -->|ActiveSync| Nginx
...
graph TD
    ...
    subgraph "Public Internet"
        External_Senders["外部发件人<br />(Gmail/Outlook)"]
        Roaming_Users["移动/桌面用户<br />(手机/PC)"]
    end

    %% --- Cloud Infrastructure ---
    subgraph "Cloud VPS (Backup MX)"
        PMG["Proxmox Mail Gateway<br />(MX Priority 20)"]
        note_pmg[功能: 杀毒/反垃圾/维护期缓存队列]
    end
    ...
    External_Senders -- "1. 正常投递 (MX 10)" --> Firewall
    External_Senders -- "2. IDC断网时 (MX 20)" --> PMG
    PMG -.-> |"3. 维护结束自动回传 (SMTP)"| Firewall
...
graph TD
    ...
    subgraph "Local IDC Infrastructure"
        ...
        %% 边界层
        FW["🔥 Firewall / Router<br />(NAT Mapping)"]:::firewall

        %% 核心网关层 (The Gatekeeper)
        PMG{{"🛡️ Proxmox Mail Gateway<br />(Anti-Spam / Anti-Virus / Stats)"}}:::gateway

        %% 内部信任网络 (Internal Trust Zone)
    ...
    %% --- 1. 入站邮件流 (Inbound: Red Lines) ---
    %% 外部 -> 防火墙 -> PMG -> Postfix -> Cyrus
    ...
    %% --- 2. 出站邮件流 (Outbound: Blue Lines) ---
    %% 内部 -> Postfix -> PMG -> 防火墙 -> 外部
    ...
    %% --- 4. 内部集成与审计 (Internal Logic: Dotted Lines) ---
    %% 身份认证
...
graph TD
    ...
    subgraph "Local IDC Infrastructure"
        ...
        %% 边界层
        FW["🔥 Firewall / Router<br />(Port Mapping & SSL Termination)"]:::firewall

        %% 核心网关层 (The Gatekeeper)
        PMG{{"🛡️ Proxmox Mail Gateway<br />(Enforced TLS / Anti-Virus)"}}:::gateway
    ...
    %% --- 3. 客户端加密访问流 (Secure Client Access: Green Lines) ---
    %% 漫游用户 -> 防火墙 (加密端口)
...
flowchart TD
    ...
    %% ================= 外部世界 (Internet) =================
    subgraph Internet ["☁️ Public Internet (Untrusted)"]
        direction TB
        External_MTA("🌍 External MTAs<br />(Gmail/Outlook)"):::external
        Roaming_Client("📱 Roaming Clients<br />(Outlook/iPhone)"):::external
    end

    %% ================= 边界防护层 (Edge / DMZ) =================
    subgraph Edge_Layer ["🔒 Edge Security Zone (DMZ)"]
        direction TB
        Firewall("🔥 Firewall / NAT<br />(Ports: 25, 443, 465, 587, 993)"):::edge
        PMG("🛡️ Proxmox Mail Gateway<br />(MTA Traffic Filter)"):::edge
    end

    %% ================= 内部核心层 (Internal) =================
    subgraph Internal_LAN ["🏢 Trusted Internal LAN"]
        direction TB
        
        %% 代理与路由
        Nginx("🌐 Nginx Proxy<br />(Webmail / JMAP)"):::secure
        Postfix("xx Postfix MTA<br />(SASL Auth / Routing)"):::secure
        
        %% 存储与业务
        Cyrus("mailbox Cyrus IMAP<br />(Mail Store)"):::storage
        SOGo("📅 SOGo Groupware<br />(Contacts / Cal)"):::storage
        
        %% 身份源
        AD("🔑 Active Directory<br />(Identity Source)"):::storage
    end
    ...
    %% ================= 2. 客户端发信链路 (Client Submission) =================
    %% 客户端 -> 防火墙 -> Postfix (认证) -> PMG (签名/扫描) -> Internet
    ...
    %% ================= 3. 客户端收信与访问 (Client Access) =================
    %% 客户端读取邮件 (IMAPS) 和 Webmail (HTTPS)
...
flowchart TD
    ...
    %% ================= 1. 外部世界 (Public Internet) =================
    subgraph Internet ["☁️ Public Internet"]
        direction TB
        Sender("🌍 External Senders<br />(Gmail/Outlook)"):::public
        Roaming("📱 Roaming User<br />(Client App)"):::public
    end

    %% ================= 2. 边界层 (Edge / DMZ) =================
    subgraph DMZ ["🔒 Local IDC Edge"]
        FW("🔥 Firewall / NAT Router<br />[Ports: 25, 465, 587, 993, 443]"):::firewall
        PMG("🛡️ Proxmox Mail Gateway<br />[Anti-Spam | Anti-Virus | Stats]"):::pmg
    end

    %% ================= 3. 内部核心层 (Trusted LAN) =================
    subgraph LAN ["🏢 Internal Trusted Network"]
        direction TB
        
        %% 核心服务
        Nginx("🌐 Nginx Proxy<br />(SSL Offloading)"):::core
        Postfix("📮 Postfix MTA<br />(Routing & Auth)"):::core
        
        %% 存储服务
        subgraph Cyrus_Cluster ["📦 Cyrus IMAP Server"]
            direction TB
            User_Box("User Mailboxes<br />(INBOX)"):::storage
            Shadow_Box("🗄️ Shadow Archive<br />(Audit Copy)"):::feature
        end
        
        SOGo("📅 SOGo Groupware"):::storage
        AD("🔑 Active Directory"):::storage

        %% 特性服务 (Features)
        Welcome_Bot("🤖 Python Welcome Bot<br />(Auto-Send Script)"):::feature
    end

    %% ================= 4. 连线逻辑 (Traffic Flows) =================

    %% --- A. 入站流 (Inbound - Red) ---
    %% 外部 -> PMG -> Postfix -> User Mailbox
    ...
    %% --- C. 特性流: 邮件归档 (Feature - Purple) ---
    %% 只要经过 Postfix，都会分叉一份到 Shadow Box
    Postfix -.-> |"🟣 Always BCC (Transparent)"| Shadow_Box
    linkStyle 11 stroke:#9c27b0,stroke-width:2px,stroke-dasharray: 5 5;

    %% --- D. 访问流: 读信与网页 (Access - Green) ---
