...
│  1. 将要写入的元数据记录到日志区                                          │
│     ┌─────────────────────────────────────────────────────────────┐     │
│     │  Journal Area: [TxID][inode block][dir block][COMMIT]       │     │
│     └─────────────────────────────────────────────────────────────┘     │
│                                 ↓                                       │
│  2. 日志提交 (COMMIT 写入) → 事务完成                                     │
│                                 ↓                                       │
│  3. 将数据从日志 checkpoint 到实际位置                                    │
│                                 ↓                                       │
│  4. 释放日志空间                                                         │
│                                                                         │
│  崩溃恢复: 重放未 checkpoint 的已提交事务                                  │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
│  ┌──────────┐                   ┌──────────┐                           │
│  │ 用户态    │ ── syscall ──→   │ 内核态    │                           │
│  │ 写入 SQ  │ io_uring_enter   │ 处理请求  │                           │
│  └──────────┘                   └──────────┘                           │
│                                                                         │
│  SQPOLL 模式:                                                           │
│  ┌──────────┐                   ┌──────────────────────┐               │
│  │ 用户态    │ (无 syscall)      │ 内核 sqpoll 线程      │               │
│  │ 写入 SQ  │ ←────────────────→│ 持续轮询 SQ          │               │
│  │          │  共享内存          │ 发现请求立即处理      │               │
│  └──────────┘                   └──────────────────────┘               │
│                                                                         │
