...
### 4.10 线程模型深究: CLONE 标志

#### 4.10.1 进程 vs 线程的内核视图
...
**EEVDF (Earliest Eligible Virtual Deadline First)** 是 Linux 6.6 引入的 CFS 替代者, 解决了 CFS 的延迟不确定性问题.

#### 6.3.1 CFS 的局限性

| 问题 | 描述 |
|:---:|------|
| **延迟不确定** | vruntime 只保证长期公平, 短期延迟无保证 |
| **唤醒延迟** | 刚唤醒的进程可能等待较长时间 |
| **交互响应** | 需要依赖启发式 (如 sleeper bonus) |

#### 6.3.2 EEVDF 核心概念

```mermaid
graph TD
    subgraph EEVDF ["EEVDF 调度决策"]
        VD[Virtual Deadline] --> Eligible{是否 Eligible?}
        Eligible -- "lag >= 0" --> Pick["选择最早 VD"]
        Eligible -- "lag < 0" --> Wait["等待变为 eligible"]
    end
```

| 概念 | 公式 | 说明 |
|:---:|------|:---:|
| **Virtual Runtime** | `vruntime` | 同 CFS, 累计加权运行时间 |
| **Lag** | `lag = V - vruntime` | V 为全局虚拟时间, lag > 0 表示欠债 |
| **Eligible** | `lag >= 0` | 进程有资格被调度 |
| **Virtual Deadline** | `VD = vruntime + slice/weight` | 虚拟截止时间 |

#### 6.3.3 调度决策

```
1. 从所有 eligible 进程中选择
2. 选择 Virtual Deadline 最早的进程
3. 如果无 eligible 进程, 选择 lag 最大者
```

#### 6.3.4 EEVDF vs CFS 对比

| 特性 | CFS | EEVDF |
|:---:|-----|-------|
| **公平性保证** | 长期公平 | 短期 + 长期公平 |
| **延迟边界** | 无硬性保证 | 有虚拟截止时间 |
| **唤醒处理** | sleeper bonus (启发式) | lag 自然处理 |
| **实现复杂度** | 较低 | 略高 |
| **交互响应** | 需要调优 | 更可预测 |
