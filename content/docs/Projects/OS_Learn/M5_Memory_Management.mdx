...
### 2.6 Slab / SLUB 分配器架构 (Kernel Allocator)

Slab 通过维护不同状态的对象桶(Cache), 实现极致的小对象分配性能, 消除内部碎片: 

```mermaid
graph TD
    Cache[Generic Cache - kmalloc-32] --> SlabNodes
    subgraph SlabNodes ["Slab Nodes (NUMA Aware)"]
...
```c
// Slab API 示例
struct kmem_cache *my_cache = kmem_cache_create("my_struct", sizeof(struct my_struct), ...);
...
### 15.1 段页式 (Segmentation with Paging)
...
### 20.1 kswapd vs 直接回收 (Direct Reclaim)
...
## 22. 高级内存抽象: Folios 与 Userfaultfd

### 22.1 Linux Folios (复合页抽象)
...
### 23.1 分级存储与 CXL (Compute Express Link)
...
# 将 CXL 内存配置为系统 RAM (memory tiering)
daxctl reconfigure-device --mode=system-ram dax0.0

# 查看 NUMA 节点 (CXL 会作为独立节点)
numactl -H
...
# 查看分层统计
cat /sys/devices/system/node/node*/vmstat | grep -E "pgpromote|pgdemote"
```

**Promotion/Demotion 机制**:

```mermaid
graph LR
    Hot["Hot Pages<br/>(Local DRAM)"] <--> |"MGLRU 降级"| Cold["Cold Pages<br/>(CXL Memory)"]
    Cold --> |"访问时升级"| Hot
    
    style Hot fill:#ceead6
    style Cold fill:#fef7e0
```

| 参数 | 路径 | 说明 |
|:---:|------|:---:|
| 启用降级 | `/sys/kernel/mm/numa/demotion_enabled` | 1=启用 |
| 降级目标 | `/sys/devices/system/node/nodeX/memory_tier` | 指定层级 |
| 统计指标 | `pgpromote_success`, `pgdemote_kswapd` | 监控升降级频率 |
...
### 23.4 memfd_secret(): 安全内存隔离 (Linux 5.14+)

`memfd_secret()` 系统调用创建一块**完全隔离的内存区域**, 即使内核也无法直接访问.

#### 23.4.1 核心机制

| 特性 | 说明 |
|:---:|------|
| **内核不可见** | 从内核直接映射中移除, 防止内核漏洞泄露 |
| **分页隔离** | 使用独立页表, 不参与常规页面管理 |
| **抗侧信道** | 减少 Spectre/Meltdown 类攻击面 |
| **进程独占** | 仅创建进程可访问, 不可跨进程共享 |
...
int main() {
    ...
    if (fd < 0) {
        perror("memfd_secret not supported");
        return 1;
    }
    ...
    // 存储敏感数据 (如加密密钥)
    memcpy(secret, "SECRET_KEY_DATA", 16);
    
    // 使用后清理
    memset(secret, 0, 4096);
    munmap(secret, 4096);
    close(fd);
    return 0;
}
...
|:---:|------|
| **密钥存储** | 加密密钥, TLS 会话密钥 |
| **凭证保护** | 数据库密码, API Token |
| **安全飞地** | 用户态 TEE 替代方案 |
| **机密计算** | 配合 AMD SEV / Intel TDX 使用 |
...
# 运行时检查
dmesg | grep -i secretmem

# 需要内核 5.14+ 且启用 CONFIG_SECRETMEM=y
```

> **安全提示**: `memfd_secret()` 提供的保护优于普通 `mlock()`, 但不能替代硬件级别的机密计算 (如 AMD SEV-SNP, Intel TDX).
