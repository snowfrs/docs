...
Git 提供了丰富的环境变量来控制底层行为, 这些是编写高级脚本和调试的利器.

### 7.1 位置控制

| 变量 | 描述 | 用途 |
| :--- | :--- | :--- |
| `GIT_DIR` | `.git` 目录的路径 | 在非仓库目录执行 Git 命令 |
| `GIT_WORK_TREE` | 工作区根目录 | 分离 Git 目录与工作区 |
| `GIT_INDEX_FILE` | Index 文件路径 | 使用临时 Index 构建提交 |
...
```bash
# 创建一个过去日期的提交
GIT_AUTHOR_DATE="2000-01-01 12:00:00" git commit -m "Ancient commit"
```
...
| 变量 | 值 | 效果 |
| :--- | :--- | :--- |
| `GIT_TRACE` | `1` 或 `true` | 显示常规跟踪信息 (命令扩展) |
| `GIT_TRACE_PACKET` | `1` | 显示网络包级跟踪 (fetch/push 调试) |
| `GIT_CURL_VERBOSE` | `1` | 显示 HTTP 通信详情 (甚至 header) |
| `GIT_TRACE_PERFORMANCE` | `1` | 显示性能计时数据 |
