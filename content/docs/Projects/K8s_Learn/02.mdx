...
### 4.3 架构演进: Ingress vs Gateway API

Gateway API 并非仅仅是 API 的升级, 而是角色的解耦与职责的重定义.

```mermaid
graph TD
    subgraph "Legacy Ingress"
        I1[User] --> I2[Ingress Controller]
        I2 --> I3[Service]
        I3 --> I4[Pod]
        note1["配置耦合: 路由、TLS、策略杂糅在 Ingress 对象中"]
    end
    
    subgraph "Modern Gateway API"
        G1[User] --> G2["Gateway (Infra Admin)"]
        G2 --> G3["HTTPRoute (App Dev)"]
        G3 --> G4[Service]
        G4 --> G5[Pod]
        note2["角色解耦: Infra 管网关资源, Dev 管路由策略"]
    end
```

### 4.4 定义 GatewayClass 与 Gateway
...
### 5.1.1 性能与资源压测对比 (Benchmark)

基于 Istio 官方与 Cilium 社区的压测数据 (Mesh 规模: 1000 Pods):

| 指标 | Sidecar (Envoy) | Ambient (ztunnel) | 提升幅度 |
| :--- | :--- | :--- | :--- |
| **CPU 开销** | ~500 vCPU (0.5 vCPU/Pod) | ~20 vCPU (DaemonSet) | **96%** 节省 |
| **内存开销** | ~50 GB (50MB/Pod) | ~2 GB (DaemonSet) | **96%** 节省 |
| **P90 延迟** | +2.8 ms / hop | +0.2 ms / hop | **90%** 降低 |
| **L7 处理** | 强制开启 (全量) | 按需开启 (Waypoint) | 弹性调度 |

> **数据解读**:
> *   **Sidecar 模式**: 即使不需要 L7 策略, Envoy 也会处理完整的 HTTP 解析, 导致显著的 CPU/内存开销.
> *   **Ambient 模式**: ztunnel 仅处理 L4 TCP/mTLS (基于 eBPF/Rust), 效率接近内核态转发. 只有在明确定义 L7 策略 (如 HTTPRoute) 时, 流量才会跳转到 Waypoint Proxy, 实现 "Pay as you go" (按需付费) 的性能模型.
